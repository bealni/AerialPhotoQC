...
namespace AerialPhotoQC
{
    public class LinWriter
    {
        ...
        private String ReadWrite()
        {
            ...
            m_PntsX = new double[100];
            m_PntsY = new double[100];
            m_Parts = new int[10];
            m_Parts[0] = 0;

            Res = m_InSHP.GetShpInfo_Start(m_sInFileName,
                                           double.MinValue,
                                           double.MinValue,
                                           double.MaxValue,
                                           double.MaxValue,
            ...
            while (true)
            {
                if (!FirstGot)
                {
                    Res = m_InSHP.GetShpInfo_First(ref m_nPntsCount,
                                                   ref m_nPartsCount);
                    if (!Res)
                        return "Error Reading Shape File " + m_sInFileName + ".";
                    FirstGot = true;
                ...
                {
                    Res = m_InSHP.GetShpInfo_Next(ref m_nPntsCount,
                                                  ref m_nPartsCount);
                    if (!Res)
                        break;
                }
                ...
                // System.Windows.Forms.MessageBox.Show(FldVals);
                Res = m_OutSHP.Polygon_AddRec(m_nPartsCount, m_nPntsCount, m_Parts, m_PntsX, m_PntsY, FldVals);
                if (!Res)
