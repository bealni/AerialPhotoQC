...
namespace AerialPhotoQC
{
    ...
    {
        private const String NAME_FLD_NAME = "NAME";
        ...
        private String DoOpen_Pnt()
        {
            ...
            {
                if (FldNames[FldIdx].ToUpper() == NAME_FLD_NAME)
                    NAME_FldIdx = FldIdx;
                else
            ...
            if (NAME_FldIdx == -1)
            {
                ...
                SHP.Dispose();
                SHP = null;
                return "Error Opening Points Shape File: Field '" + NAME_FLD_NAME + "' does not exist.";
            }
            ...
            Util.GEO2UTMZone(0.5 * (MinY + MaxY),
                             0.5 * (MinX + MaxX),
                             out m_nUTMZone,
                             out m_nUTMNS);
            ...
            Res = SHP.GetShpInfo_Start(m_sPntFileName,
                                       double.MinValue,
                                       double.MinValue,
                                       double.MaxValue,
                                       double.MaxValue,
            ...
            while (true)
            {
                ...
                sName = SHP.GetShpInfo_DBF(NAME_FldIdx);
                sElevation = SHP.GetShpInfo_DBF(ELEVATION_FldIdx);
                if (!int.TryParse(sName, out Name))
                {
                    FldNames = null;
                    FldTypes = null;
                    FldLens = null;
                    ...
                    PntsY = null;
                    Parts = null;
                    return "Error Reading Points Shape File: Field '" + NAME_FLD_NAME + "' is not an integer number.";
                }
        ...
        private String DoOpen_Lin()
        {
            ...
            if (ShapeType != ShpLib.SHPTYPE_POLYLINE)
            {
                FldNames = null;
                FldTypes = null;
                FldLens = null;
                ...
                SHP.Dispose();
                SHP = null;
                return "Error Opening Lines Shape File: Shape Type must be POLYLINE.";
            }
            ...
            if (ELEVATION_FldIdx == -1)
            {
                ...
                SHP.Dispose();
                SHP = null;
                return "Error Opening Lines Shape File: Field '" + ELEVATION_FLD_NAME + "' does not exist.";
            }
            ...
            Res = SHP.GetShpInfo_Start(m_sLinFileName,
                                       double.MinValue,
                                       double.MinValue,
                                       double.MaxValue,
                                       double.MaxValue,
            ...
            PntsX = new double[100];
            PntsY = new double[100];
            Parts = new int[10];
            Parts[0] = 0;
            while (true)
            {
                ...
                if (!double.TryParse(sElevation, out Elevation))
                {
                    ...
                    PntsY = null;
                    Parts = null;
                    return "Error Reading Lines Shape File: Field '" + ELEVATION_FLD_NAME + "' is not a real number.";
                }
        ...
        private String DoOpen_AssignLines()
        {
            ...
            for (i = 0; i < Count; i++)
            {
                ...
                for (j = 0; j < Count; j++)
                {
                    ...
                    lidx = m_LinXs[j].Count - 1;

                    if (Math.Min(Math.Abs(m_PntXs[i][0] - m_LinXs[j][0]),
                                 Math.Abs(m_PntXs[i][0] - m_LinXs[j][lidx])) > CLOSE_TOL)
                        continue;
                    if (Math.Min(Math.Abs(m_PntXs[i][pidx] - m_LinXs[j][0]),
                                 Math.Abs(m_PntXs[i][pidx] - m_LinXs[j][lidx])) > CLOSE_TOL)
                        continue;

                    if (Math.Min(Math.Abs(m_PntYs[i][0] - m_LinYs[j][0]),
                                 Math.Abs(m_PntYs[i][0] - m_LinYs[j][lidx])) > CLOSE_TOL)
                        continue;
                    if (Math.Min(Math.Abs(m_PntYs[i][pidx] - m_LinYs[j][0]),
                                 Math.Abs(m_PntYs[i][pidx] - m_LinYs[j][lidx])) > CLOSE_TOL)
                        continue;

                    if (Math.Abs(m_PntZs[i][0] - m_LinZs[j][0]) > CLOSE_TOL)
                        continue;

                    m_PntLinIdxs[i] = j;
                    m_LinPntIdxs[j] = i;

                    if (Math.Abs(m_PntXs[i][0] - m_LinXs[j][0]) > Math.Abs(m_PntXs[i][0] - m_LinXs[j][lidx]))
                    {
                        m_LinXs[j].Reverse();
                        m_LinYs[j].Reverse();
                        m_LinZs[j].Reverse();
                        if (m_LinKappas[j] > 0.0)
                            m_LinKappas[j] -= Math.PI;
                        else
                            m_LinKappas[j] += Math.PI;
                    }
        ...
        private String DoOpen_AssignNeighbourLines()
        {
            ...
            for (i = 0; i < Count; i++)
            {
                if (m_LinLeftIdxs[i] == -1)
                {
                    Idx = DoOpen_AssignNeighbourLines_Left(i, out dist);
                    if (Idx != -1)
                    {
                        m_LinLeftIdxs[i] = Idx;
                        m_LinRightIdxs[Idx] = i;
                        m_LinLeftDist[i] = dist;
                        m_LinRightDist[Idx] = dist;
                    }
                }
                if (m_LinRightIdxs[i] == -1)
                {
